...
MemoryReader.inputQueue = {} -- 按键队列
MemoryReader.currentInput = nil -- 当前帧要执行的按键
...
function MemoryReader.update()
    ...
    -- 处理按键队列
    MemoryReader.processInput()
end

-- 处理按键队列
function MemoryReader.processInput()
    -- 如果有当前帧要执行的按键，先释放
    if MemoryReader.currentInput then
        joypad.set({})
        MemoryReader.currentInput = nil
    end
    
    -- 如果队列中有按键，取出下一个
    if #MemoryReader.inputQueue > 0 then
        MemoryReader.currentInput = table.remove(MemoryReader.inputQueue, 1)
        joypad.set(MemoryReader.currentInput)
    end
end

-- 添加按键到队列
function MemoryReader.enqueueInput(buttons)
    table.insert(MemoryReader.inputQueue, buttons)
end
