...
function DataConverter.getBattleData(memoryReader)
    if not memoryReader.isInitialized then
        return {error = "Memory reader not initialized"}
    end

    if not memoryReader.partyReader then
        return {error = "Party reader not available"}
    end

    if memoryReader.currentGame.gameInfo.generation ~= 3 and memoryReader.currentGame.gameInfo.generation ~= "CFRU" then
        return {error = "Battle data reading not supported for this generation"}
    end

    if not memoryReader.currentGame.addresses.gBattleMons then
        return {error = "Battle Mons address not available"}
    end

    local gameUtils = require("utils.gameutils")
    local battleMonsAddr = gameUtils.hexToNumber(memoryReader.currentGame.addresses.gBattleMons)
    local battleParty = {}
    local pokemonData = require("readers.pokemondata")

    for i = 0, 3 do
        local pokemon = memoryReader.partyReader:readBattlePokemon(battleMonsAddr, i)
        if pokemon and pokemon.speciesID > 0 then
            -- Build moves arrays (only include non-zero moves)
            local moves = {}
            local moveNames = {}
            ...
            table.insert(battleParty, apiPokemon)
        end
    end
...
function DataConverter.getEnemyData(memoryReader)
    if not memoryReader.isInitialized then
        return {error = "Memory reader not initialized"}
    end
    ...
    if memoryReader.currentGame.gameInfo.generation == 3 or memoryReader.currentGame.gameInfo.generation == "CFRU" then
        if not memoryReader.currentGame.addresses.enemyPartyAddr then
            return {error = "Enemy party address not available"}
        end
        local gameUtils = require("utils.gameutils")
        local enemyPartyAddr = gameUtils.hexToNumber(memoryReader.currentGame.addresses.enemyPartyAddr)
        enemyParty = memoryReader.partyReader:readEnemyParty({enemyPartyAddr = enemyPartyAddr}, gameCode)
    else
        return {error = "Enemy party reading not supported for this generation"}
    end
...
function DataConverter.getMapData(memoryReader)
    if not memoryReader.isInitialized then
        return {error = "Memory reader not initialized"}
    end

    if memoryReader.currentGame.gameInfo.generation ~= 3 and memoryReader.currentGame.gameInfo.generation ~= "CFRU" then
        return {error = "Map data reading not supported for this generation"}
    end

    local Gen3MapReader = require("readers.map.gen3mapreader")
    local mapReader = Gen3MapReader:new()
    local mapData = mapReader:readMap(memoryReader.currentGame.addresses)
