...
function Gen3PartyReader:readBattlePokemon(battleMonsAddr, battlerId)
    local battlePokemonStructSize = 0x58
    local opponentBase = battleMonsAddr + (battlerId * battlePokemonStructSize)
    
    local speciesId = gameUtils.read16(opponentBase + 0x00)
    if speciesId == 0 then
        return nil
    end
    
    local attack = gameUtils.read16(opponentBase + 0x02)
    local defense = gameUtils.read16(opponentBase + 0x04)
    local speed = gameUtils.read16(opponentBase + 0x06)
    local spAttack = gameUtils.read16(opponentBase + 0x08)
    local spDefense = gameUtils.read16(opponentBase + 0x0A)
    
    local ivs = gameUtils.read32(opponentBase + 0x14)
    local hpIV = self:getBits(ivs, 0, 5)
    local attackIV = self:getBits(ivs, 5, 5)
    local defenseIV = self:getBits(ivs, 10, 5)
    local speedIV = self:getBits(ivs, 15, 5)
    local spAttackIV = self:getBits(ivs, 20, 5)
    local spDefenseIV = self:getBits(ivs, 25, 5)
    
    local move1 = gameUtils.read16(opponentBase + 0x0C)
    local move2 = gameUtils.read16(opponentBase + 0x0E)
    local move3 = gameUtils.read16(opponentBase + 0x10)
    local move4 = gameUtils.read16(opponentBase + 0x12)
    
    local pp1 = gameUtils.read8(opponentBase + 0x24)
    local pp2 = gameUtils.read8(opponentBase + 0x25)
    local pp3 = gameUtils.read8(opponentBase + 0x26)
    local pp4 = gameUtils.read8(opponentBase + 0x27)
    
    local ability = gameUtils.read8(opponentBase + 0x20)
    
    local type1 = gameUtils.read8(opponentBase + 0x21)
    local type2 = gameUtils.read8(opponentBase + 0x22)
    
    local hp = gameUtils.read16(opponentBase + 0x28)
    local level = gameUtils.read8(opponentBase + 0x2A)
    local friendship = gameUtils.read8(opponentBase + 0x2B)
    local maxHP = gameUtils.read16(opponentBase + 0x2C)
    local heldItemId = gameUtils.read16(opponentBase + 0x2E)
    
    local nicknameBytes = gameUtils.readBytes(opponentBase + 0x30, 11)
    local nickname = charmaps.decryptText(nicknameBytes, "GBA")
    
    local experience = gameUtils.read32(opponentBase + 0x44)
    local personality = gameUtils.read32(opponentBase + 0x48)
    local otId = gameUtils.read32(opponentBase + 0x54)
    
    local status1 = gameUtils.read32(opponentBase + 0x4C)
    local status = 0
    local sleepTurns = 0

    if status1 <= 0 then
        status = 0
    elseif status1 < 8 then
        status = 1
    elseif status1 == 8 then
        status = 2
    elseif status1 == 16 then
        status = 3
    elseif status1 == 32 then
        status = 4
    elseif status1 == 64 then
        status = 5
    elseif status1 == 128 then
        status = 6
    end
    ...
    local speciesName = self:getSpeciesName(speciesId)
    
    local abilityID = 0
    local abilityName = "Unknown"
    if speciesData then
        local abilityNum = self:getBits(ivs, 30, 1)
    ...
    local types = {type1Name}
    if type2Name and type2Name ~= type1Name then
    ...
    return {
        ...
        ppBonuses = gameUtils.read8(opponentBase + 0x3B),
        friendship = friendship,
        ivHP = hpIV,
        ivAttack = attackIV,
        ivDefense = defenseIV,
        ivSpeed = speedIV,
        ivSpAttack = spAttackIV,
        ivSpDefense = spDefenseIV,
        move1 = move1,
        move2 = move2,
        move3 = move3,
        move4 = move4,
        ...
        type1Name = type1Name,
        type2Name = type2Name,
        hiddenPower = self:calculateHiddenPowerType(ivs),
        hiddenPowerName = pokemonData.getHiddenPowerName(self:calculateHiddenPowerType(ivs)),
