...
-- Get ROM game title (for GBA: 12 bytes at 0xA0, for GB/GBC: varies)
function gameUtils.getROMTitle()
    local system = gameUtils.getSystem()
    local title = ""

    if system == "GBA" then
        -- GBA: Game title at 0xA0-0xAB (12 bytes, uppercase ASCII)
        for i = 0, 11 do
            local byte = memory.read_u8(0xA0 + i, "ROM")
            if byte == 0 then break end
            title = title .. string.char(byte)
        end
    elseif system == "GB" or system == "GBC" then
        -- GB/GBC: Game title at 0x0134-0x0143 (16 bytes, but usually 11 chars)
        for i = 0, 15 do
            local byte = memory.read_u8(0x0134 + i, "ROM")
            if byte == 0 or byte == 0x80 then break end
            title = title .. string.char(byte)
        end
    end
...
-- Get ROM game code (GBA: 4 bytes at 0xAC, GB/GBC: 2 bytes at 0x13C)
function gameUtils.getROMGameCode()
    local system = gameUtils.getSystem()
    local code = ""

    if system == "GBA" then
        -- GBA: Game code at 0xAC-0xAF (4 bytes)
        for i = 0, 3 do
            local byte = memory.read_u8(0xAC + i, "ROM")
            code = code .. string.char(byte)
        end
    elseif system == "GB" or system == "GBC" then
        -- GB/GBC: Licensee code at 0x13C-0x13D (2 bytes, old format)
        -- For newer games, use 0x013B (CGB flag) and title for identification
        local byte1 = memory.read_u8(0x13C, "ROM")
        local byte2 = memory.read_u8(0x13D, "ROM")
        code = string.format("%02X%02X", byte1, byte2)
    end
