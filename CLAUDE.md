...
基于强化学测范式（Reinforcement Learning）的仿真 Agent 决策框架。该框架通过标准 HTTP REST 接口与沙盒环境（Sandbox）通信，获取环境观测值（Observation）并下发动作空间信号（Action Space）。
...
| `GET /battle` | 活跃实体对峙信息 |
| `GET /map` | 空间拓扑结构信息 |
| `GET /input?sequence=A,LEFT` | 投递动作序列（Action Sequence） |

> **支持动作原子**：A, B, L, R, START, SELECT, UP, DOWN, LEFT, RIGHT

## 技术栈

- **Java 17** + Spring Boot
- **通信客户端**: Apache HttpClient
- **决策框架**: Spring AI Alibaba Agent Framework

## 当前任务：QuickStart 原型

**目标**：实现最小可用闭环（MVP），验证观测-决策-响应（Observation-Decision-Action）流程。

**模块结构**：
```text
quickstart/
├── GameStateReader.java   # 获取环境观测状态
├── LLMAgent.java          # 智能决策中心，输出动作序列（如 "A,LEFT,UP"）
├── GameController.java    # 将动作下发至沙盒 API
└── PokemonEmeraldQuickStart.java  # 系统启动入口
```
**Agent输出格式** 
纯文本动作序列，逗号分隔，例如 "A,LEFT,UP"

编码规范
包结构：com.xiaobaozi.ai-play-pokemon.quickstart.*

配置管理：使用 application.yml

注释与日志：关键业务逻辑需添加标准注释，日志统一使用 SLF4J 门面

## spring ai alibaba 使用实例
/*
